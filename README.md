<h3 align="center">CSS Project - Stripe Website</h3>

<p align="center"> 💳 A CSS practice project showcasing layout, styling components, and responsive design using plain HTML & CSS.
    <br> 
</p>

<p align="center">
  <a href="https://dorarodriguezag.github.io/css-project/">
    <img src="https://img.shields.io/badge/Live-Demo-brightgreen" alt="Live Demo">
  </a>
  <img src="https://img.shields.io/badge/HTML5-%23E34F26.svg?style=flat&logo=html5&logoColor=white" alt="HTML5">
  <img src="https://img.shields.io/badge/CSS3-%231572B6.svg?style=flat&logo=css3&logoColor=white" alt="CSS3">
</p>

## 📝 Table of Contents

- [About](#about)
- [Getting Started](#getting_started)
- [Features](#features)
- [Prerequisites](#prerequisites)
- [Clone the Repository](#clone_repository)
- [Folder Structure](#folder_structure)
- [How to Use / Customize](#customize)
- [Technologies Used](#technologies)
- [Goals for Future Versions](#future_versions)
- [License](#license)

## 🧐 About <a name = "about"></a>

This project was built to practice and demonstrate CSS layout skills, typography, responsive design, utility classes, and UI styling without any JavaScript or frameworks.

The goal is to create a clean and modern user interface using only HTML and CSS — perfect for improving fundamentals and portfolio showcase.

## 🏁 Getting Started <a name = "getting_started"></a>

### Features <a name = "features"></a>

- Semantic HTML structure
- Clean and organized CSS
- Responsive layout for different screen sizes
- Custom utility classes
- Navigation bar and hero section
- Styled content sections with cards, buttons, and forms
- Hover & transition effects for UI enhancement

### ⚡ Prerequisites <a name = "prerequisites"></a>

All you need is a modern browser — no installations required.

### 📥 Clone the Repository <a name = "clone_repository"></a>

git clone https://github.com/dorarodriguezag/css-project.git

## 🔧 Folder Structure <a name = "folder_structure"></a>

    css-project/
    ├── assets/
    ├── gradient.js
    ├── index.css
    ├── index.html
    └── README.md

### 🎨 How to Use / Customize <a name = "customize"></a>

- Add new sections by copying the HTML structure
- Extend styles in index.css
- Create reusable CSS utility classes
- Experiment with CSS Grid and Flexbox

### 🛠 Technologies Used <a name = "technologies"></a>

- HTML5: Markup of the pages
- CSS3: Styling and layout
- Git/GitHub: Version control & deployment

## 🚀 Goals for Future Versions <a name = "future_versions"></a>

- Dark & light theme toggle
- Animations with CSS keyframes
- Improved mobile layout breakpoints

## 📄 License <a name = "license"></a>

This project is open source — feel free to use.
